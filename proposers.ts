...
export function llmProposer<T extends string = string>(task: string): Proposer<T> {
  return async ({ stageName, pastAttempts, dataSummary, initialPrompts }): Promise<Prompt> => {
    ...
    if (stageName === "generate") {
      systemPrompt = `your goal is to improve prompts for the given task (${task}) based on past performance.
